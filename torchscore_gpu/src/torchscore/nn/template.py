...
from torchscore import features, scoring, protein_data_processing
from torchscore import geometry
from torchscore.nn import pairformer
...
def make_backbone_rigid(
    ...
    rigid_mask = (torch.index_select(mask, dim=0, index=a)
                  * torch.index_select(mask, dim=0, index=b)
                  * torch.index_select(mask, dim=0, index=c)).diag()
    frame_positions = []
    for indices in [a, b, c]:
        frame_positions.append(
            geometry.Vec3Array(
                x=torch.index_select(positions.x.permute(1, 0), dim=0, index=indices).diag(),
                y=torch.index_select(positions.y.permute(1, 0), dim=0, index=indices).diag(),
                z=torch.index_select(positions.z.permute(1, 0), dim=0, index=indices).diag(),
            )
        )

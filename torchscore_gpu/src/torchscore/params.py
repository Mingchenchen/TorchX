...
def LinearHMAParams_k(l, out_features_per_head, already_transpose_weights=False):
    d = {"weights": LinearWeightMHA(l.weight[out_features_per_head:2*out_features_per_head, :], already_transpose_weights)}
    return d
...
def GridSelfAttentionParams(pair_attention): return {
    ...
    "q_projection": LinearHMAParams_q(pair_attention.qkv_projection, pair_attention.c_pair, already_transpose_weights=True),
    "k_projection": LinearHMAParams_k(pair_attention.qkv_projection, pair_attention.c_pair, already_transpose_weights=True),
...
def import_jax_weights_(model, model_path: pathlib.Path):
    try:
        params = get_torchfold_params(model_path / "af3.bin")
    except Exception:
        params = get_torchfold_params(model_path / "af3.bin.zst")

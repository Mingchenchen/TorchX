...
def structure_to_array(structure: Structure, chain_ids: List[str] = None) -> np.ndarray:
    """Converts structure to a coordinate array. Each residue is mapped to a 24-atom slot based on ATOM14 order."""
    model = structure[0]
    all_coords_list = []
    ...
    # Concatenate all chains: [Total_Residues, 24, 3]
...
def save_traced_array(
    traced_array: jax.Array, seq_length: int, save_path: str, metadata: Dict = None
) -> None:
...
def process_pdb_folder(
    ...
    # Retry failed files (often handles transient I/O issues)
    if failed_paths:
        ...
        for failed_file in failed_paths:
            ...
            retry_args.append((failed_file, output_path, chain_ids, num_copies))
        ...
        retry_success = [path for success, path in retry_results if success]
        retry_fail = [path for success, path in retry_results if not success]
...
def main():
    ...
    parser.add_argument(
        "--pdb_folder", type=str, required=True, help="Input directory containing PDB files"
    )
    parser.add_argument(
        "--output_folder", type=str, required=True, help="Output directory for H5 files"
    )
    parser.add_argument(
        "--num_workers", type=int, default=4, help="Number of parallel processes (default: 4)"
    )
    ...
    try:
        ...
        bucket = 3072
        print(f"Warning: Could not detect bucket from folder name, using default: {bucket}")

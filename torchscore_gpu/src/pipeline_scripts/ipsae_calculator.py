...
def load_tf_pae_and_chains(
    json_path: Union[str, Path], pdb_path: Union[str, Path]
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Extracts PAE matrix, chain IDs, and residue types from TorchFold output files.

    Logic:
    1. Parse PDB to identify residues corresponding to TorchFold Tokens (typically CA or C1').
    ...
    # Compatibility check for different TorchFold JSON key naming conventions
    if "pae" in data:
        raw_pae = np.array(data["pae"])
    elif "predicted_aligned_error" in data:
        raw_pae = np.array(data["predicted_aligned_error"])
    else:
        # Handle cases where TorchFold output might be a list containing the data
...
if __name__ == "__main__":
    # Example paths
    pdb_file = Path("/path/to/example.pdb")
    json_file = Path("/path/to/confidences.json")

    if pdb_file.exists() and json_file.exists():
        try:
            print(f"Processing: {pdb_file} ...")
            pae, chains, res_types = load_tf_pae_and_chains(
                json_file, pdb_file
            )

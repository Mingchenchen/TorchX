...
def save_config(config: Union[ConfigDict, dict], path: str) -> None:
    ...
    # Lustre OSTs occasionally return EIO on close/flush even after the bytes
    # land on disk, which used to kill rank 0 inside init_basics. The yaml is
    # bookkeeping only — retry with an atomic tmp+rename, and downgrade to a
    # warning rather than crash an 8-GPU run.
    import errno, logging, os, tempfile, time
    tmp_dir = os.path.dirname(path) or "."
    for attempt in range(3):
        try:
            fd, tmp_path = tempfile.mkstemp(
                prefix=".config.", suffix=".yaml.tmp", dir=tmp_dir
            )
            try:
            ...
            if e.errno == errno.EIO and attempt < 2:
                time.sleep(1 + attempt)
                continue
            logging.warning(
                "save_config failed to write %s: %s — training continues.",
                path, e,
            )
            return

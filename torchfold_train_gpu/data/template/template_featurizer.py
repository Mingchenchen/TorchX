...
from torchfold.data.msa.msa_utils import map_to_standard
from torchfold.data.template.template_parser import HHRParser, HmmsearchA3MParser
from torchfold.data.template.template_utils import (
...
from torchfold.data.utils import pad_to
from torchfold.utils.file_io import load_json_cached
...
def _lookup_entity_value(
    values: Mapping[Any, Any], entity_id: Any, default: Any = None
) -> Any:
    # 0713: Custom AbAg pkl files may store sequence keys with a different entity-id type.
    candidates = [entity_id, str(entity_id)]
    try:
...
def _lookup_entity_sequence(sequences: Mapping[Any, str], entity_id: Any) -> Optional[str]:
    return _lookup_entity_value(sequences, entity_id)


def _sequence_from_asym_tokens(
    atom_array: AtomArray, token_array: Any, asym_id: int
) -> str:
    # 0713: Recover missing polymer sequences from token centers instead of treating them as unknown ligands.
    try:
        centre_atom_indices = token_array.get_annotation("centre_atom_index")
        centre_atoms = atom_array[centre_atom_indices]
        asym_mask = centre_atoms.asym_id_int == asym_id
        if not bool(np.any(asym_mask)):
            return ""
        if hasattr(centre_atoms, "cano_seq_resname"):
            res_names = centre_atoms.cano_seq_resname[asym_mask]
        else:
            res_names = centre_atoms.res_name[asym_mask]
        chars = []
        for res_name in res_names:
            text = str(res_name).strip().upper()
            chars.append(mmcif_restype_3to1.get(text, text if len(text) == 1 else "X"))
        return "".join(chars)
    except Exception:
        return ""


def _parse_query_release_date(value: Any, pdb_id: str) -> Optional[datetime]:
    # 0713: AbAg design pkls can store release_date=0; skip query-date filtering instead of crashing.
    if value is None:
        return None
    ...
    if text.lower() in {"", "0", "nan", "none", "na", "<na>"}:
        return None
    try:
        return datetime.strptime(text, "%Y-%m-%d")
    except (TypeError, ValueError):
        logger.warning(
            "Skip query release-date filtering for %s because release_date=%r is invalid",
            pdb_id,
            value,
        )
        return None
...
class TemplateFeatureAssemblyLine:
    """
    Orchestrates the conversion of raw templates into finalized features.
...
class TemplateFeaturizer:
    ...
    def make_template_features(
        ...
        entity_id_to_sequence = {
            eid: _lookup_entity_sequence(sequences, eid)
        ...
        for aid, eid in asym_to_entity_id.items():
            ...
            if seq is None:
                seq = _sequence_from_asym_tokens(atom_array, token_array, aid)
        ...
        if self.stage == "train":
            query_release_date = _parse_query_release_date(
                bioassembly_dict.get("release_date"), pdb_id
            )
        ...
        for aid, sequence in asym_id_to_sequence.items():
            ...
            chain_type = poly_types_mapping.get(
                _lookup_entity_value(entity_poly_type, eid, "non-polymer"),
                LIGAND_CHAIN_TYPES,
            )
...
class Templates:
    ...
    def as_standard_dict(self) -> BatchDict:
        """Compute additional features and return as dictionary."""

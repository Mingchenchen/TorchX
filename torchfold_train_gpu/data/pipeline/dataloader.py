...
from torchfold.data.pipeline.dataset import Dataset, get_datasets

from torchfold.utils.logger import get_logger
from torchfold.utils.torch_utils import collate_fn_first
...
def _worker_kwargs(num_workers) -> dict:
    """DataLoader kwargs that are only legal when num_workers > 0.

    timeout: a worker killed by the OS (e.g. host-RAM OOM while featurizing a
      large templated complex) is not a Python exception, so without a timeout
      the main process polls the dead worker's SimpleQueue forever (silent
      deadlock). A positive timeout turns that into a raised RuntimeError.
    persistent_workers: keep workers alive across epochs / _evaluate calls
      instead of respawning (which would re-pay worker startup every eval).
    prefetch_factor: prefetch upcoming samples so the CPU-bound template/MSA
      featurization overlaps with the GPU forward.
    """
    if num_workers and num_workers > 0:
        return dict(timeout=1200, persistent_workers=True, prefetch_factor=1)  # 1200s: abag template search (8.7M hmmsearch.a3m + mmCIF + kalign) legitimately takes ~100-300s/complex
    return {}
...
def get_dataloaders(
    ...
    if world_size > 1:
        ...
        train_dl = DistributedDataLoader(
            ...
            **_worker_kwargs(configs.data.num_dl_workers),
        )
    else:

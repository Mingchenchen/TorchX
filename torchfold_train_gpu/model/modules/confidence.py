"""torchfold.model.modules.confidence
...
# atom_layout.convert + GatherInfo: used in _embed_features for pseudo-beta positions.
from torchfold.model.modules import atom_layout

# atom_types.DENSE_ATOM_NUM: scalar constant (max dense atom count per residue).
from torchfold.model.constants import atom_types

# DistogramFeaturesConfig + dgram_from_positions already ported into torchfold.
from torchfold.model.modules.embedders import DistogramFeaturesConfig, dgram_from_positions

# Vendored LayerNorm (replaces fastnn.LayerNorm)
from torchfold.model.triangular.layers import LayerNorm

# PairformerBlock already ported
from torchfold.model.modules.pairformer import PairformerBlock


class ConfidenceHead(nn.Module):
    """Confidence head (Algorithm 31 in AF3).

    AF3 layout:
      - left_target_feat_project / right_target_feat_project: 447 → c_pair
      - distogram_feat_project: DistogramFeaturesConfig.num_bins → c_pair
      - confidence_pairformer: ModuleList of n_pairformer_layers PairformerBlock
      - logits heads: left_half_distance_logits, pae_logits, plddt_logits,
        experimentally_resolved_logits (each preceded by LayerNorm)
    """

    def __init__(self, c_single: int = 384, c_pair: int = 128,
                 c_target_feat: int = 447, n_pairformer_layers: int = 4):
        ...
        self.logits_ln = LayerNorm(self.c_pair)
        ...
        self.pae_logits_ln = LayerNorm(self.c_pair)
        ...
        self.plddt_logits_ln = LayerNorm(self.c_single)
        self.plddt_logits = nn.Linear(
            self.c_single, self.num_atom * self.num_plddt_bins, bias=False)

        self.experimentally_resolved_ln = LayerNorm(self.c_single)
    ...
    def _embed_features(
        ...
        out = out + self.distogram_feat_project(dgram.to(torch.float32))
    ...
    def forward(
        ...
        Args:
            dense_atom_positions (torch.Tensor): array of positions.
                [N_tokens, N_atom, 3]
            embeddings (dict): must contain keys 'pair', 'single', 'target_feat'
            seq_mask (torch.Tensor): sequence mask [N_token]
            token_atoms_to_pseudo_beta (atom_layout.GatherInfo): pseudo-beta gather info
            asym_id (torch.Tensor): asymmetric unit IDs [N_token]

        Returns:
            dict with keys: predicted_lddt, predicted_experimentally_resolved,
                full_pde, average_pde, full_pae, tmscore_adjusted_pae_global,
        ...
        pair_act = embeddings['pair'].detach().to(dtype=dtype)
        single_act = embeddings['single'].detach().to(dtype=dtype)
        target_feat = embeddings['target_feat'].detach().to(dtype=dtype)
        ...
        for layer in self.confidence_pairformer:
            if self.training:
                pair_act, single_act = torch.utils.checkpoint.checkpoint(
                    layer, pair_act, pair_mask, single_act, seq_mask,
                    use_reentrant=False,
                )
            else:
        ...
        return {
            ...
            **pae_outputs,
            # --- Per-bin logits (ADDITIVE; same tensors used above for the
            #     expectations, just also returned so a strict-AF3 cross-entropy
            #     confidence loss can read them). NOT involved in any expectation
            #     output above, so this changes no existing value / parameter.
            #   plddt_logits:  [*, N_token, num_atom(=DENSE_ATOM_NUM=24), 50]
            #   pae_logits:    [*, N_token, N_token, 64]   (<=31 Å bins)
            #   pde_logits:    [*, N_token, N_token, 64]   (symmetrized distance
            #                  logits, exactly the tensor whose softmax-expectation
            #                  becomes full_pde; <=31 Å bins)
            #   experimentally_resolved_logits: [*, N_token, num_atom, 2]
            'plddt_logits': plddt_logits,
            'pae_logits': pae_logits,
            'pde_logits': distance_logits,
    ...
    def _get_tmscore_adjusted_pae(self,
        ...
        num_global_tokens = num_global_tokens * seq_mask.sum()

...
from torchfold.utils.permutation.utils import save_permutation_error
...
def run(
    ...
    **kwargs,
    ...
    try:
        ...
        # --- diagnostic: identify the failing sample + test the symmetric/unresolved hypothesis ---
        try:
            _pdb = input_feature_dict.get("pdb_id", "?")
            _asym = input_feature_dict["mol_id"]
            _ent = input_feature_dict["entity_mol_id"]
            _cm = label_full_dict["coordinate_mask"]
            _sym = {int(_e): int(torch.unique(_asym[_ent == _e]).numel())
                    for _e in torch.unique(_ent).tolist()}
            _symdup = {k: v for k, v in _sym.items() if v > 1}
            logger.warning(
                f"[perm-diag] pdb={_pdb} chains={int(torch.unique(_asym).numel())} "
                f"entities={len(_sym)} symmetric_copies_gt1={_symdup} "
                f"GT_resolved_frac={float(_cm.float().mean().item()):.3f} fail={type(e).__name__}"
            )
        except Exception:
            pass

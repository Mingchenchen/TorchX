...
import os
_USE_CUEQ_TRIATT = os.environ.get("TRIANGLE_ATTENTION", "").lower() == "cuequivariance"
    ...
    def _attention(self, pair: torch.Tensor, mask: torch.Tensor, bias: torch.Tensor):
        ...
        if _USE_CUEQ_TRIATT and q.is_cuda:
            ...
            if mask is not None:
                ...
                if mb.dim() == 1:
                    cu_mask = mb[None, None, None, :]
                elif mb.dim() == 2:
                    cu_mask = mb[:, None, None, :]
                else:
                    cu_mask = mb
            weighted_avg = _cueq_triatt_fn()(
                q.contiguous(), k.contiguous(), v.contiguous(),
                bias[None].to(torch.float32).contiguous(),
                mask=cu_mask, scale=scale,
            )[0]
        else:

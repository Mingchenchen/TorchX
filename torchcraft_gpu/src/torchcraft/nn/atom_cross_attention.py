...
from torchcraft import feat_batch
from torchcraft.nn import atom_layout, utils
from torchcraft.nn.diffusion_transformer import DiffusionCrossAttTransformer
...
class AtomCrossAttEncoder(nn.Module):
    ...
    def _per_atom_conditioning(self, batch: feat_batch.Batch) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        act = act + self.embed_ref_mask(batch.ref_structure.mask.unsqueeze(2).to(
        ...
        act = act + self.embed_ref_charge(torch.arcsinh(
        ...
        act = act * batch.ref_structure.mask.unsqueeze(2)
        ...
        pair_act = pair_act + self.embed_pair_offsets(batch.ref_structure.positions.unsqueeze(2)
        ...
        pair_act = pair_act + self.embed_pair_distances(1.0 /
    ...
    def forward(
        ...
        if self.first_run:
            ...
            pair_act = pair_act + self.embed_pair_offsets_valid(offsets_valid.unsqueeze(3).to(
        ...
        if token_atoms_act is None:
            queries_act = self.queries_single_cond.clone()
        else:
            ...
            queries_act = queries_act + self.queries_single_cond

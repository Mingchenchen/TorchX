...
from torchcraft.nn import template, atom_layout, pairformer
...
class ConfidenceHead(nn.Module):
    ...
    def forward(
        ...
        if use_gradient_checkpointing:
            for layer in self.confidence_pairformer:
                pair_act, single_act = CheckpointReentrantFunction.apply(layer, pair_act, pair_mask, single_act, seq_mask)
        else:
            for layer in self.confidence_pairformer:
    ...
    def _get_tmscore_adjusted_pae(self,
        ...
        def get_tmscore_adjusted_pae(num_interface_tokens, bin_centers, pae_probs):
            ...
            d0 = d0.unsqueeze(2)
            bin_centers = bin_centers.unsqueeze(0).unsqueeze(1)

...
# task args
flags.DEFINE_enum('task_type', 'monomer', ['monomer', 'mini_binder', 'vhh'], ' task type')
...
def main(argv):
    ...
    # mini_binder and vhh tasks need to preprocess pdb files to generate json files
    if FLAGS.task_type in ['mini_binder', 'vhh']:
        if not FLAGS.pdb_path:
            print("Error: mini_binder and vhh task must provide pdb file --pdb_path")
            sys.exit(1)

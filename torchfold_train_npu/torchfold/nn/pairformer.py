...
class PairformerBlock(nn.Module):
    ...
    def forward(
        ...
        pair = pair + self.pair_attention1(pair, mask=pair_mask)
        pair = pair + self.pair_attention2(pair, mask=pair_mask)
        ...
        if self.with_single is True:
            ...
            if pair_logits.dim() == 3:
                ...
                pair_logits = pair_logits.permute(0, 3, 1, 2).contiguous()
            single = single + self.single_attention_(single,
                                             seq_mask,
...
class EvoformerBlock(nn.Module):
    ...
    def forward(
        ...
        msa = msa + self.msa_attention1(msa, msa_mask, pair)

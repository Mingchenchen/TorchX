...
class SelfAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Use local variable to avoid keeping unnecessary references
        # This doesn't reduce memory for the tensor itself (it's still in computation graph),
        # but avoids module-level references that might interfere with garbage collection
        padded_pair_logits = pair_logits
        ...
        if q.dim() == 2:
            ...
            weighted_avg = fastnn.dot_product_attention(
                q, k, v, mask=mask, bias=padded_pair_logits
            )
            weighted_avg = weighted_avg.squeeze(0)
        else:
            q, k, v = map(lambda t: einops.rearrange(
                t, '... n (h c) -> ... h n c', h=self.num_head), [q, k, v])
...
class DiffusionTransformer(nn.Module):
    def __init__(self,
        ...
        self.pair_logits_projection = nn.ModuleList(
            [nn.Linear(self.c_pair_cond, self.super_block_size * self.num_head, bias=False) for _ in
             range(self.num_super_blocks)])
    ...
    def forward(self,
        ...
        # Update: drop caching and recompute every forward pass
        ...
        for super_block_i in range(self.num_super_blocks):
            ...
            # Note: no need to detach unless you explicitly want to block gradients w.r.t. pair_cond.
            # If pair_cond is connected to Evoformer and requires gradients, keep it attached.
        ...
        for super_block_i in range(self.num_super_blocks):
            for j in range(self.super_block_size):
                ...
                act = act + self.transition_block[super_block_i *
...
class CrossAttention(nn.Module):
    def __init__(self, key_dim: int = 128, value_dim: int = 128, c_single_cond: int = 128, num_head: int = 4) -> None:
        ...
        # No longer hardcode bfloat16; let q_scale automatically adapt based on input dtype in forward
        self.register_buffer('q_scale', torch.tensor([self.key_dim_per_head ** (-0.5)]))
    ...
    def forward(
        ...
        # Use input dtype instead of hardcoded bfloat16 to be compatible with AMP
...
class DiffusionCrossAttTransformer(nn.Module):
    def __init__(self, c_query: int = 128, c_single_cond: int = 128, c_pair_cond: int = 16, num_blocks: int = 3,
                 num_head: int = 4) -> None:
        ...
        self.transition_block = nn.ModuleList(
            [DiffusionTransition(c_x=self.c_query, c_single_cond=self.c_single_cond, use_single_cond=True) for _ in
    ...
    def forward(
        ...
        # Update: remove cached logits and operate on local tensors
        pair_act = self.pair_input_layer_norm(pair_cond)
        ...
        # Removed: self.pair_logits = pair_logits
        ...
        for block_idx in range(self.num_blocks):
            ...
            queries_act = queries_act + self.cross_attention[block_idx](
                ...
                mask_q=queries_mask,
                mask_k=keys_mask,
                pair_logits=pair_logits[block_idx, ...],  # Use local tensor

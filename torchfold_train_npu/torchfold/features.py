...
class GroundTruthStructure:
    """Contains ground truth structure information."""

    true_positions: torch.Tensor
    true_positions_atom_mask: torch.Tensor

    @classmethod
    def from_data_dict(cls, batch: BatchDict) -> Self:
        # For inference mode, true_positions may not exist
        # Create placeholder tensors if missing
        if 'true_positions' not in batch:
            # Create empty placeholder tensors for inference mode
            true_positions = torch.zeros((0, 3), dtype=torch.float32)
            true_positions_atom_mask = torch.zeros((0,), dtype=torch.bool)
        else:
            true_positions = batch['true_positions']
            true_positions_atom_mask = batch['true_positions_atom_mask']

        return cls(
            true_positions=true_positions,
            true_positions_atom_mask=true_positions_atom_mask,
        )

    def as_data_dict(self) -> BatchDict:
        return {
            'true_positions': self.true_positions,
            'true_positions_atom_mask': self.true_positions_atom_mask,
        }

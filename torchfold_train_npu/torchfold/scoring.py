...
def pseudo_beta_fn(
    ...
    pseudobeta_index_polymer = torch.take_along_dim(
        rigid_utils.RESTYPE_PSEUDOBETA_INDEX.to(device=aatype.device), aatype.to(dtype=torch.int64), axis=0
    ...
    pseudo_beta = torch.take_along_dim(
        dense_atom_positions, pseudobeta_index.unsqueeze(-1).unsqueeze(-1), dim=-2
    )
    pseudo_beta = torch.squeeze(pseudo_beta, dim=-2)

    pseudo_beta_mask = torch.take_along_dim(
        dense_atom_masks, pseudobeta_index.unsqueeze(-1), dim=-1

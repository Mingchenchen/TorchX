...
from . import atom_layout, utils
from .diffusion_transformer import DiffusionCrossAttTransformer
...
def _gather_row_sharded_trunk_pair_to_atom_pair(
    trunk_pair_cond_row: torch.Tensor,
    tokens_to_queries: atom_layout.GatherInfo,
    tokens_to_keys: atom_layout.GatherInfo,
    parallel_spec,
) -> torch.Tensor:
    """Gather row-sharded trunk pair into atom-layout pair conditioning.

    The result is equivalent to first materializing full [L, L, C] pair and then
    applying atom_layout.convert, but avoids the full pair tensor. Each rank
    contributes values for its local query-token rows and an all-reduce sums the
    non-overlapping contributions into the full atom-layout tensor.
    """
    query_global = tokens_to_queries.gather_idxs
    key_global = tokens_to_keys.gather_idxs

    local_rows = trunk_pair_cond_row.shape[0]
    global_cols = trunk_pair_cond_row.shape[1]

    query_valid = (
        tokens_to_queries.gather_mask.to(torch.bool)
        & (query_global >= parallel_spec.start)
        & (query_global < parallel_spec.end)
        & (query_global < parallel_spec.n_global)
    )
    key_valid = (
        tokens_to_keys.gather_mask.to(torch.bool)
        & (key_global >= 0)
        & (key_global < global_cols)
        & (key_global < parallel_spec.n_global)
    )

    local_query = (query_global - parallel_spec.start).clamp(0, max(local_rows - 1, 0))
    key_index = key_global.clamp(0, max(global_cols - 1, 0))

    pair = trunk_pair_cond_row[
        local_query.unsqueeze(2),
        key_index.unsqueeze(1),
    ]

    valid = query_valid.unsqueeze(2) & key_valid.unsqueeze(1)
    pair = pair * valid.unsqueeze(-1).to(dtype=pair.dtype)

    if getattr(parallel_spec, "is_distributed", False):
        if not _dist_is_initialized():
            raise RuntimeError("Parallel spec is distributed but torch.distributed is not initialized.")
        dist.all_reduce(pair, op=dist.ReduceOp.SUM, group=parallel_spec.group)
...
class AtomCrossAttEncoder(nn.Module):
    def __init__(self,
        ...
        self.first_run = True
        self.queries_mask = None
        self.queries_single_cond = None
        self.keys_mask = None
        self.keys_single_cond = None
        self.pair_act = None
    ...
    def _per_atom_conditioning(self, batch: feat_batch.Batch) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        act += self.embed_ref_mask(batch.ref_structure.mask.unsqueeze(2).to(
            dtype=self.embed_ref_mask.weight.dtype))

        # Atomic numbers are embedded into 128 classes.
        ...
        act += self.embed_ref_charge(torch.arcsinh(
            batch.ref_structure.charge).unsqueeze(2))

        # Atom-name characters use ASCII-minus-32 encoding with 64 classes.
        ...
        act *= batch.ref_structure.mask.unsqueeze(2)
        ...
        pair_act += self.embed_pair_offsets(batch.ref_structure.positions.unsqueeze(2)
                                            - batch.ref_structure.positions.unsqueeze(1))

        sq_dists = torch.sum(
            torch.square(
                batch.ref_structure.positions.unsqueeze(2)
                - batch.ref_structure.positions.unsqueeze(1)
            ),
            dim=-1,
        )
        pair_act += self.embed_pair_distances(1.0 /
                                              (1 + sq_dists.unsqueeze(3)))
    ...
    def forward(
        ...
        if self.first_run:
            ...
            assert trunk_pair_cond is None or self.with_trunk_pair_cond
        ...
        if self.first_run:
            ...
            if trunk_pair_cond is not None:
                ...
                if parallel_spec is not None:
                    pair_act += _gather_row_sharded_trunk_pair_to_atom_pair(
                        trunk_pair_cond,
                    ...
                    # Create the GatherInfo into a flattened trunk_pair_cond from
                    # the queries and keys gather infos.
            ...
            offsets_valid = (
                queries_ref_space_uid.unsqueeze(2) == keys_ref_space_uid.unsqueeze(1)
            )
            offsets = queries_ref_pos.unsqueeze(2) - keys_ref_pos.unsqueeze(1)

            pair_act += (self.embed_pair_offsets_1(offsets)
                        * offsets_valid.unsqueeze(3))

            sq_dists = torch.sum(torch.square(offsets), dim=-1)
            pair_act += self.embed_pair_distances_1(
                1.0 / (1 + sq_dists.unsqueeze(3))) * offsets_valid.unsqueeze(3)
            pair_act += self.embed_pair_offsets_valid(offsets_valid.unsqueeze(3).to(
            ...
            self.pair_act = pair_act

        queries_mask = self.queries_mask
        queries_single_cond = self.queries_single_cond
        keys_mask = self.keys_mask
        keys_single_cond = self.keys_single_cond
        pair_act = self.pair_act
        ...
        if token_atoms_act is None:
            ...
            queries_act *= queries_mask.unsqueeze(-1)
            queries_act += queries_single_cond
...
class AtomCrossAttDecoder(nn.Module):
    ...
    def forward(self,
        ...
        queries_act *= enc.queries_mask.unsqueeze(-1)

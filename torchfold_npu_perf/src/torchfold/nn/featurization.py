...
from torchfold.nn import utils
...
def gumbel_noise(
    ...
    """
    # Use the global generator so model seeding controls this sample.
...
def create_relative_encoding(
    ...
    *,
    ...
    """Write relative features directly into one contiguous output.

    Keep the integer default for existing callers; model consumers request
    their floating dtype to avoid an intermediate dense int64 encoding.
    Only 0/1 feature values are written, with the original bin definitions.
    """
    ...
    position_bins = 2 * max_relative_idx + 2
    chain_bins = 2 * max_relative_chain + 2
    rel_feat = torch.zeros(
        (token_index.shape[0], token_index.shape[0],
         2 * position_bins + 1 + chain_bins),
        dtype=dtype, device=token_index.device,
    )
    ...
    rel_feat.scatter_(-1, final_offset.to(dtype=torch.int64).unsqueeze(-1), 1)
    ...
    rel_feat.scatter_(
        -1, (final_token_offset.to(dtype=torch.int64) + position_bins).unsqueeze(-1), 1
    )

    # Embed same entity ID
    entity_id_same = left_entity_id == right_entity_id
    rel_feat[..., 2 * position_bins].copy_(entity_id_same)
    ...
    rel_feat.scatter_(
        -1, (final_rel_chain.to(dtype=torch.int64) + 2 * position_bins + 1).unsqueeze(-1), 1
    )

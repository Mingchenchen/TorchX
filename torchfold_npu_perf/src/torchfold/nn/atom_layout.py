...
def convert(
    ...
    *,
    ...
    if layout_axes_begin == 0:
        ...
        batch_size = arr_flattened.shape[0]
        idx_flat = gather_info.gather_idxs.reshape(-1)
        gather_shape = gather_info.gather_idxs.shape
        features_shape = arr_flattened.shape[2:]

        out_flat = torch.stack(
            [
                mx_driving.npu_index_select(
                    arr_flattened[batch_idx],
                    dim=0,
                    index=idx_flat,
                )
        ...
        out_arr = out_flat.reshape(
            (batch_size,) + gather_shape + features_shape
        )

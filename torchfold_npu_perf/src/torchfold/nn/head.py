...
from torchfold.nn import template, atom_layout, pairformer
...
class DistogramHead(nn.Module):
    ...
    def forward(
        ...
        pair_act = embeddings['pair'].to(
            device=self.half_logits.weight.device,
        )
        seq_mask = batch.token_features.mask.to(dtype=torch.bool)
        pair_mask = seq_mask.unsqueeze(1) * seq_mask.unsqueeze(0)

        left_half_logits = self.half_logits(pair_act)
        del pair_act
        right_half_logits = left_half_logits
        right_half_logits = right_half_logits.transpose(-2, -3).contiguous()
        left_half_logits.add_(right_half_logits)
        del right_half_logits
        probs = torch.softmax(left_half_logits, dim=-1)
        del left_half_logits
...
class ConfidenceHead(nn.Module):
    ...
    def forward(
        ...
        pair_act = embeddings['pair'].to(
            device=dense_atom_positions.device,
            copy=True,
        ).to(dtype=dtype)
        ...
        del (
            left_distance_logits,
            right_distance_logits,
            distance_logits,
            distance_probs,
        )
        ...
        del pae_probs, pair_mask_bool, pair_mask

...
from torchfold.nn.attention import GridSelfAttention, MSAAttention
from torchfold.nn.diffusion_transformer import SelfAttention
from torchfold.nn.layer_norm import LayerNorm
from torchfold.nn.primitives import Transition, OuterProductMean
from torchfold.nn.triangle_multiplication import TriangleMultiplication


from torchfold.runtime_policy import (
    TRIANGLE_MULTIPLICATION_CHUNK_SIZE,
    TRIANGLE_ATTENTION_CHUNK_SIZE,
    PAIRFORMER_TRIANGLE_FULL_PATH_MAX_TOKENS,
    inference_chunking,
    pairformer_triangle_chunked,
)
...
class PairformerBlock(nn.Module):
    ...
    def forward(
        ...
        if not inference_chunking(training=self.training, grad_enabled=torch.is_grad_enabled()):
        ...
        elif not pairformer_triangle_chunked(
            feature_length=pair.shape[-3], training=self.training,
            ...
            self.triangle_multiplication_outgoing.forward_residual_axis_chunked_(
                pair,
                pair_mask,
                TRIANGLE_MULTIPLICATION_CHUNK_SIZE,
            )
            self.triangle_multiplication_incoming.forward_residual_axis_chunked_(
                pair,
                pair_mask,
                TRIANGLE_MULTIPLICATION_CHUNK_SIZE,
            )
            self.pair_attention1.add_residual_batch_chunked_(
                pair,
                pair_mask,
                TRIANGLE_ATTENTION_CHUNK_SIZE,
            )
            self.pair_attention2.add_residual_batch_chunked_(
                pair,
                pair_mask,
                TRIANGLE_ATTENTION_CHUNK_SIZE,
            )
        ...
        if self.with_single is True:
            ...
            pair_logits = pair_logits.permute(2, 0, 1).contiguous()

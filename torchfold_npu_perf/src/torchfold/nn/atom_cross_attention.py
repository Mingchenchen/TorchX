...
class AtomCrossAttEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.first_run or self.training or torch.is_grad_enabled():
            assert (trunk_pair_cond is not None) == self.with_trunk_pair_cond
        else:
            assert trunk_pair_cond is None or self.with_trunk_pair_cond
            if self.with_trunk_pair_cond and trunk_pair_cond is None:
                if self.pair_act is None:
                    raise ValueError('missing Atom encoder pair cache')
        ...
        if token_atoms_act is None:
            ...
            queries_act *= self.queries_mask.unsqueeze(-1)
            queries_act += self.queries_single_cond

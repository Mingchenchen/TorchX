...
def get_torchfold_params(checkpoint_path: pathlib.Path):
    ...
    for k, param in translation_dict.items():
        with torch.no_grad():
            ...
            if param.stacked:
                ...
                # Flatten both leading dimensions for two-level stacked weights.
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to assign model parameter: {k}"
                ) from exc


class ParamType(Enum):
    # partial keeps transformations as Enum values instead of bound methods.
    LinearWeight = partial(
        lambda w: w.transpose(-1, -2).contiguous()
    )
    LinearWeightMHA = partial(
        lambda w: w.reshape(*w.shape[:-2], -1).transpose(-1, -2).contiguous()
    )
...
def LinearHMAParams_q(l, out_features_per_head, use_bias=False, already_transpose_weights=False):
    d = {"weights": LinearWeightMHA(l.weight[:out_features_per_head, :], already_transpose_weights)}
    if use_bias:
        d["bias"] = LinearBiasMHA(l.bias[:out_features_per_head])
    return d


def LinearHMAParams_k(l, out_features_per_head, already_transpose_weights=False):
    d = {
        "weights": LinearWeightMHA(
            l.weight[
                out_features_per_head : 2 * out_features_per_head,
                :,
            ],
            already_transpose_weights,
        )
    }
    return d


def LinearHMAParams_v(l, out_features_per_head, already_transpose_weights=False):
    d = {"weights": LinearWeightMHA(l.weight[2*out_features_per_head:, :], already_transpose_weights)}
    return d
...
def GridSelfAttentionParams(pair_attention): return {
    ...
    "q_projection": LinearHMAParams_q(
        pair_attention.qkv_projection,
        pair_attention.c_pair,
        already_transpose_weights=True,
    ),
    "k_projection": LinearHMAParams_k(
        pair_attention.qkv_projection,
        pair_attention.c_pair,
        already_transpose_weights=True,
    ),
    "v_projection": LinearHMAParams_v(pair_attention.qkv_projection, pair_attention.c_pair),
...
def SelfAttentionParams(self_attention, use_single_cond=False):
    return {
        "q_projection": LinearHMAParams_q(self_attention.qkv_projection, self_attention.c_x, use_bias=True),
        "k_projection": LinearHMAParams_k(self_attention.qkv_projection, self_attention.c_x),
        "v_projection": LinearHMAParams_v(self_attention.qkv_projection, self_attention.c_x),
...
def AtomCrossAttEncoderParams(encoder,
    ...
    d = {
        ...
        "atom_transformer_encoder": DiffusionCrossAttTransformerParams(
            encoder.atom_transformer_encoder,
            prefix=prefix,
        ),
...
def AtomCrossAttDecoderParams(decoder): return {
    ...
    "atom_transformer_decoder": DiffusionCrossAttTransformerParams(
...
def TemplateEmbeddingParams(template_embedding):
    single_template_embedding = template_embedding.single_template_embedding
    pairformer_params = stacked(
        [
            PairformerBlockParams(block, with_single=False)
            for block in single_template_embedding.template_embedding_iteration
        ]
    )

    return {
        "single_template_embedding/query_embedding_norm": LayerNormParams(
            single_template_embedding.query_embedding_norm
        ),
        "single_template_embedding/template_pair_embedding_0": LinearParams(
            single_template_embedding.template_pair_embedding_0
        ),
        "single_template_embedding/template_pair_embedding_1": LinearfromFlatParams(
            single_template_embedding.template_pair_embedding_1
        ),
        "single_template_embedding/template_pair_embedding_2": LinearParams(
            single_template_embedding.template_pair_embedding_2
        ),
        "single_template_embedding/template_pair_embedding_3": LinearParams(
            single_template_embedding.template_pair_embedding_3
        ),
        "single_template_embedding/template_pair_embedding_4": LinearfromFlatParams(
            single_template_embedding.template_pair_embedding_4
        ),
        "single_template_embedding/template_pair_embedding_5": LinearfromFlatParams(
            single_template_embedding.template_pair_embedding_5
        ),
        "single_template_embedding/template_pair_embedding_6": LinearfromFlatParams(
            single_template_embedding.template_pair_embedding_6
        ),
        "single_template_embedding/template_pair_embedding_7": LinearfromFlatParams(
            single_template_embedding.template_pair_embedding_7
        ),
        "single_template_embedding/template_pair_embedding_8": LinearParams(
            single_template_embedding.template_pair_embedding_8
        ),
        **cat_params(
            pairformer_params,
            "single_template_embedding/__layer_stack_no_per_layer/"
            "template_embedding_iteration/",
        ),
        "single_template_embedding/output_layer_norm": LayerNormParams(
...
def import_jax_weights_(model, model_path: pathlib.Path):
    candidates = ("af3.bin", "af3.bin.zst", "torchfold.bin")
    weight_path = next(
        (model_path / name for name in candidates if (model_path / name).is_file()),
        None,
    )
    if weight_path is None:
        raise FileNotFoundError(
            f"No supported model weight found in {model_path}: {candidates}"
        )
    params = get_torchfold_params(weight_path)

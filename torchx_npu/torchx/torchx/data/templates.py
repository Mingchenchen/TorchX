...
from torchx.common import resources
...
from torchx.data import msa_config
from torchx.data import parsers
from torchx.data import structure_stores
from torchx.data import template_realign
from torchx.data.tools import hmmsearch
...
class Hit:
    ...
    def keep(
        ...
        release_date_cutoff = datetime.strptime(release_date_cutoff, '%Y-%m-%d').date()
...
class Templates:
    ...
    def structures(self) -> Iterator[structure.Structure]:
        ...
        """
        from datetime import datetime
        release_date_cutoff = datetime.strptime(self.release_date_cutoff, '%Y-%m-%d').date()

        for hit in self.hits:
            if hit.release_date > release_date_cutoff:  # pylint: disable=comparison-with-callable
                raise HitDateError(
...
def get_polymer_features(
    ...
    if chain.release_date is None:
        from datetime import datetime
        warn_msg = (
            f"Warning: Template {chain.name} has no release date. "
            f"Using current date: {datetime.now().strftime('%Y-%m-%d')}"
        )
        print(warn_msg)
        final_date_obj = datetime.now()
    else:
        final_date_obj = chain.release_date

    release_date_str = final_date_obj.strftime('%Y-%m-%d')
    ...
    return {
        ...
        'template_release_date': np.array(release_date_str.encode(), dtype=object),
    }

...
from torchx.cpp import cif_dict

from torchx import structure
from torchx.constants import chemical_components
from torchx.constants import mmcif_names
from torchx.constants import residue_names
from torchx.structure import mmcif as mmcif_lib

BondAtomId: TypeAlias = tuple[str, int, str]

JSON_DIALECT: Final[str] = 'TorchFold'
JSON_VERSIONS: Final[tuple[int, ...]] = (1, 2, 3)
JSON_VERSION: Final[int] = JSON_VERSIONS[-1]

TORCHFOLD_SERVER_JSON_DIALECT: Final[str] = 'TorchFoldserver'
TORCHFOLD_SERVER_JSON_VERSION: Final[int] = 1
...
class ProteinChain:
  ...
  @classmethod
  def from_TorchFoldserver_dict(
      cls, json_dict: Mapping[str, Any], seq_id: str
  ) -> Self:
    """Constructs ProteinChain from the TorchFoldServer JSON dict."""
    _validate_keys(
        json_dict.keys(),
        {
            'sequence',
    ...
    if 'glycans' in json_dict:
      raise ValueError(
          f'Specifying glycans in the `{TORCHFOLD_SERVER_JSON_DIALECT}` format'
          ' is not supported.'
      )

    if 'maxTemplateDate' in json_dict:
      raise ValueError(
          f'Specifying maxTemplateDate in the `{TORCHFOLD_SERVER_JSON_DIALECT}`'
  ...
  def from_dict(
    ...
    """Constructs ProteinChain from the TorchFold JSON dict."""
  ...
  def to_dict(
    ...
    """Converts ProteinChain to an TorchFold JSON dict."""
...
class RnaChain:
  ...
  def from_TorchFoldserver_dict(
    ...
    """Constructs RnaChain from the TorchFoldServer JSON dict."""
  ...
  def from_dict(
    ...
    """Constructs RnaChain from the TorchFold JSON dict."""
  ...
  def to_dict(
    ...
    """Converts RnaChain to an TorchFold JSON dict."""
    contents = {
        'id': seq_id or self._id,
        'sequence': self._sequence,
        'modifications': [
...
class DnaChain:
  ...
  def from_TorchFoldserver_dict(
    ...
    """Constructs DnaChain from the TorchFoldServer JSON dict."""
    _validate_keys(json_dict.keys(), {'sequence', 'modifications', 'count'})
    sequence = json_dict['sequence']
    modifications = [
        (mod['modificationType'].removeprefix('CCD_'), mod['basePosition'])
  ...
  def from_dict(
    ...
    """Constructs DnaChain from the TorchFold JSON dict."""
  ...
  def to_dict(
    ...
    """Converts DnaChain to an TorchFold JSON dict."""
    contents = {
        'id': seq_id or self._id,
        'sequence': self._sequence,
        'modifications': [
...
class Ligand:
  ...
  def from_TorchFoldserver_dict(
    ...
    """Constructs Ligand from the TorchFoldServer JSON dict."""
  ...
  def from_dict(
    ...
    """Constructs Ligand from the TorchFold JSON dict."""
  ...
  def to_dict(
    ...
    """Converts Ligand to an TorchFold JSON dict."""
...
def _sample_rng_seed() -> int:
  """Sample a random seed for TorchFoldServer job."""
  # See https://TorchFoldserver.com/faq#what-are-seeds-and-how-are-they-set.
...
class Input:
  ...
  @classmethod
  def from_TorchFoldserver_fold_job(cls, fold_job: Mapping[str, Any]) -> Self:
    """Constructs Input from an TorchFoldServer fold job."""
    ...
    if 'dialect' not in fold_job and 'version' not in fold_job:
      dialect = TORCHFOLD_SERVER_JSON_DIALECT
      version = TORCHFOLD_SERVER_JSON_DIALECT
    elif 'dialect' in fold_job and 'version' in fold_job:
      dialect = fold_job['dialect']
      version = fold_job['version']
    else:
      raise ValueError(
          'TorchFold Server input JSON must either contain both `dialect` and'
          ' `version` fields, or neither. If neither is specified, it is'
          f' assumed that `dialect="{TORCHFOLD_SERVER_JSON_DIALECT}"` and'
          f' `version="{TORCHFOLD_SERVER_JSON_DIALECT}"`.'
      )

    if dialect != TORCHFOLD_SERVER_JSON_DIALECT:
      raise ValueError(
          f'TorchFold Server input JSON has unsupported dialect: {dialect}, '
          f'expected {TORCHFOLD_SERVER_JSON_DIALECT}.'
      )

    # For now, there is only one TorchFold Server JSON version.
    if version != TORCHFOLD_SERVER_JSON_DIALECT:
      raise ValueError(
          f'TorchFold Server input JSON has unsupported version: {version}, '
          f'expected {TORCHFOLD_SERVER_JSON_DIALECT}.'
      )
    ...
    for sequence in fold_job['sequences']:
      if 'proteinChain' in sequence:
        for _ in range(sequence['proteinChain'].get('count', 1)):
          chains.append(
              ProteinChain.from_TorchFoldserver_dict(
        ...
        for _ in range(sequence['rnaSequence'].get('count', 1)):
          chains.append(
              RnaChain.from_TorchFoldserver_dict(
        ...
        for _ in range(sequence['dnaSequence'].get('count', 1)):
          chains.append(
              DnaChain.from_TorchFoldserver_dict(
        ...
        for _ in range(sequence['ion'].get('count', 1)):
          chains.append(
              Ligand.from_TorchFoldserver_dict(
                  sequence['ion'],
                  seq_id=mmcif_lib.int_id_to_str_id(len(chains) + 1),
              )
          )
  ...
  def from_json(
    ...
    """Loads the input from the TorchFold JSON string."""
    ...
    if 'dialect' not in raw_json or 'version' not in raw_json:
      raise ValueError(
          'TorchFold input JSON must contain `dialect` and `version` fields.'
      )

    if raw_json['dialect'] != JSON_DIALECT:
      raise ValueError(
          'TorchFold input JSON has unsupported dialect:'
          f' {raw_json["dialect"]}, expected {JSON_DIALECT}.'
      )

    if raw_json['version'] not in JSON_VERSIONS:
      raise ValueError(
          'TorchFold input JSON has unsupported version:'
          f' {raw_json["version"]}, expected one of {JSON_VERSIONS}.'
      )

    if 'sequences' not in raw_json:
      raise ValueError('TorchFold input JSON does not contain any sequences.')

    if 'modelSeeds' not in raw_json or not raw_json['modelSeeds']:
      raise ValueError(
          'TorchFold input JSON must specify at least one rng seed in'
          ' `modelSeeds`.'
      )
    ...
    if all(raw_sequence_ids):
      ...
      raise ValueError(
          'TorchFold input JSON contains sequences with unset IDs.'
      )
  ...
  def from_mmcif(cls, mmcif_str: str, ccd: chemical_components.Ccd) -> Self:
    ...
    struc = structure.from_mmcif(
        ...
        # we have a way of handling these in the TorchFold input format.
        include_other=False,
  ...
  def to_json(self) -> str:
    """Converts Input to an TorchFold JSON."""
    deduped_chains = {}
    deduped_chain_ids = {}
    for chain in self.chains:
...
def load_fold_inputs_from_path(json_path: pathlib.Path) -> Iterator[Input]:
  ...
  if isinstance(raw_json, list):
    ...
    try:
      ...
      raise ValueError(
          f'Failed to load input from {json_path} (TorchFold dialect): {e}'
      ) from e

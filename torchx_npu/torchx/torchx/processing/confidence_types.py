...
This module provides two variants of confidence data structures:
- Standard versions (from confidence_types1.py) with rounding for JSON serialization.
- Alternative versions (from confidence_types.py) with `_NoRound` suffix, preserving original precision.

Both variants share the same base classes to minimize code duplication.
"""

import dataclasses
import enum
import json
from typing import Any, Self, Mapping, TypeAlias
...
class StructureConfidenceFullNoRoundEncoder(json.JSONEncoder):
    """JSON encoder for serializing confidence types."""
...
class ConfidenceCategory(enum.Enum):
    """Confidence categories for TorchFold predictions."""
...
class AtomConfidence:
    """Dataclass for 1D per-atom confidences from TorchFold."""
    ...
    def from_inference_result(
            cls, inference_result: InferenceResult
    ) -> Self:
        """Instantiates an AtomConfidence from a structure.

        Args:
          inference_result: Inference result from TorchFold.
    ...
    @classmethod
    def from_inference_result_no_round(
            cls, inference_result: InferenceResult
    ) -> Self:
        """Instantiates an AtomConfidence from a structure.
...
class StructureConfidenceSummary:
    """Dataclass for the summary of structure scores from TorchFold.
...
class StructureConfidenceFull:
    """Dataclass for full structure data from TorchFold."""
    ...
    def to_json_no_round(self) -> str:
        """Converts StructureConfidenceFull to json string."""
        return json.dumps(self, cls=StructureConfidenceFullNoRoundEncoder)

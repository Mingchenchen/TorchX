...
The default attention implementation is `torch`. To use CANN Fusion_Attention, install the operator (see [`../torchx_npu/README.md`](../torchx_npu/README.md)), then pass `--dot_product_attention=Fusion_Attention` to `run_torchscore.py` (or add the same flag in `src/pipeline_scripts/TorchScore_pipeline.sh`).

#### Scoring Pipeline (TorchScore)

TorchScore evaluates protein complex structures on Ascend NPU using a unified pipeline script. The pipeline takes a directory of `.pdb` files as input and produces per-chain and inter-chain confidence metrics.
...
| `PYTHON_EXEC`    | Python executable (must have torchx installed) | `python`                  |
| `TOTAL_CARDS`    | Number of Ascend NPU cards available              | `4`                       |
...
| 3    | `TorchScore_pipeline.sh`   | Runs `run_torchscore.py` on each batch across Ascend NPU cards               |

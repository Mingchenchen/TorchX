...
log_info "NPU cards       : $TOTAL_CARDS (max parallel: $MAX_JOBS)"
...
for subfolder in "$tf_input_batch/pdb"/*; do
    if [[ -d "$subfolder" ]]; then
        ...
        bash "$pipeline_script_dir/02_prepare_pdb2jax.sh" \
            ...
            "$pipeline_script_dir" > "$jax_log_dir/${folder_name}.log" 2>&1 &
...
# =================================================================
log_step "03" "Running TorchScore inference on NPU"

# NPU environment setup
# CANN toolkit. Edit below, or pass: ASCEND_TOOLKIT_HOME=/path/to/ascend-toolkit
...
for json_batch_dir in "$tf_input_batch/json"/batch_*; do
    ...
    for json_file in "$json_batch_dir"/*.json; do
        ...
        (
            export ASCEND_RT_VISIBLE_DEVICES=$card_id

...
from torchx.common import base_config
...
from torchx.processing.pipeline import inter_chain_bonds
from torchx.processing.pipeline import structure_cleaning
...
class WholePdbPipeline:
  ...
  class Config(base_config.BaseConfig):
    ...
    Properties:
      ...
      resolve_msa_overlaps: Whether to deduplicate unpaired MSA against paired
        MSA. The default behaviour matches the method described in the TorchFold
    ...
    deterministic_frames: bool = False
  ...
  def process_item(
    ...
    batch_ref_structure, ligand_ligand_bonds = (
        features.RefStructure.compute_features_parallel(
            all_token_atoms_layout=all_token_atoms_layout,
            ccd=ccd,
            padding_shapes=padding_shapes,
            chemical_components_data=chemical_components_data,
    ...
    if self._config.deterministic_frames:
      deterministic_ref_structure, _ = features.RefStructure.compute_features_parallel(
          all_token_atoms_layout=all_token_atoms_layout,
          ccd=ccd,
          padding_shapes=padding_shapes,
          chemical_components_data=chemical_components_data,
